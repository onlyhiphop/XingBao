...
window.onload = function () {
    ...
    function praiseBox(box, el) {
        ...
        var praisesTotal = box.getElementsByClassName('praises-total')[0];  //点赞的class
        var oldTotal = parseInt(praisesTotal.getAttribute('total'));   //点赞的总数
        var newTotal;
        if (txt == '赞') {
            newTotal = oldTotal + 1;
    ...
    for (var i = 0; i < boxs.length; i++) {
        ...
        boxs[i].onclick = function (e) {
            e = e || window.event;  //兼容处理 IE情况下也能正确获取到事件对象
        ...
        //评论(输入框)
        var textArea1 = document.getElementById('comment1');
       
        //评论获取焦点
        textArea1.onfocus = function () {
            this.parentNode.className = 'text-box text-box-on';
            this.value = this.value == '我的心得…' ? '' : this.value;
            this.onkeyup();
        }
		...
		xinde.onclick=function(){
	        ...
	        commentBox.innerHTML =
	            ...
	            '<img class="head" src="img/info_4.jpg" alt=""/>'+
	            '<div class="content">'+
	               ' <div class="main">'+
	                    '<p class="txt">'+
	                        '<span class="user">小Y：</span>'+textarea1.value+
	                    '</p>'+'<img class="pic" src="img/book4.png" alt=""/>'+
	                '</div>'+
